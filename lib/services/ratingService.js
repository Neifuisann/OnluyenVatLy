...
class RatingService {
  ...
  calculateRatingChange(previousRating, performance, timeTaken, streak) {
    ...
    let ratingChange = baseK * (performanceFactor - expectedScore) * timeBonus * streakMultiplier;

    // Apply performance modifiers: X2 bonus for high scores, 1.5x penalty for low scores
    if (ratingChange > 0 && performanceFactor >= 0.8) {
      ratingChange *= 2;
    } else if (ratingChange < 0 && performanceFactor <= 0.5) {
      ratingChange *= 1.5;
    }
