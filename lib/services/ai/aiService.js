...
class AIService {
  ...
  async formatDocumentWithAI(text, onChunk = null) {
...
6. QUAN TRỌNG: Giữ lại toàn bộ các đoạn văn bản dẫn, thông tin chung hoặc ngữ liệu dùng cho một nhóm câu hỏi (ví dụ: "Sử dụng thông tin sau cho Câu 1 và Câu 2...", "Đọc đoạn văn sau...", v.v.). Các đoạn này phải được đặt ngay trước câu hỏi đầu tiên của nhóm đó.

QUY TẮC CHUYỂN ĐỔI:
- Nếu văn bản có sẵn câu hỏi, TUYỆT ĐỐI GIỮ NGUYÊN nội dung và định dạng lại cho đúng chuẩn. Không được phép thay đổi câu hỏi, dữ kiện và lựa chọn bằng bất kì lí do nào. Đảm bảo chuyển đổi TẤT CẢ các câu và KHÔNG bỏ sót thông tin dẫn.
- Sử dụng latex đối với các phương trình, ký hiệu toán học hoặc hóa học trong cặp dấu $inline-latex$
...
Sử dụng thông tin sau cho Câu 2 và Câu 3: Một hình tròn có bán kính $R = 5cm$.
Câu 2: Các phát biểu sau về hình tròn này là đúng hay sai?
*a) Chu vi bằng $10\pi$ cm
b) Diện tích bằng $10\pi$ cm²
*c) Đường kính bằng $10cm$
d) Có hai tâm đối xứng

Câu 3: Tính diện tích hình tròn trên (lấy π = 3.14)
Answer: 78.5
...
OUTPUT (chỉ trả về nội dung đã định dạng, không giải thích thêm):`;
    ...
    try {
      ...
      let formattedContent = this.cleanupAIResponse(fullText);
...
Hãy trả lời một cách hữu ích và chi tiết theo định dạng Markdown:`;
   ...
   */
  /**
   * Process an image-only PDF directly via Gemini's vision/multimodal API.
   * Sends the raw PDF bytes as base64 inline data so Gemini can OCR and reformat.
   * @param {Buffer} pdfBuffer - Raw PDF file buffer
   * @returns {string} - Formatted lesson content (same format as formatDocumentWithAI)
   */
  async processImagePdfWithAI(pdfBuffer, onChunk = null) {
    const base64Pdf = pdfBuffer.toString('base64');

    const prompt = `Bạn là một trợ lý AI chuyên định dạng nội dung bài học cho hệ thống giáo dục.

NHIỆM VỤ: Đọc toàn bộ nội dung từ file PDF này (có thể là ảnh scan), rồi chuyển đổi thành định dạng bài học chuẩn với các câu hỏi trắc nghiệm.
...
QUY TẮC CHUYỂN ĐỔI:
- Nếu văn bản có sẵn câu hỏi, TUYỆT ĐỐI GIỮ NGUYÊN và định dạng lại cho đúng chuẩn. Không được thay đổi câu hỏi và lựa chọn. Đảm bảo chuyển đổi TẤT CẢ các câu.
...
OUTPUT (chỉ trả về nội dung đã định dạng, không giải thích thêm):`;
    ...
    try {
      ...
      if (!formattedContent.includes('Câu 1:')) {
        console.warn('AI response from image PDF does not contain expected question format');
        throw new Error('AI không tạo được câu hỏi từ nội dung PDF');
      }

      return formattedContent;
    } catch (error) {
      console.error('Error in processImagePdfWithAI:', error);
      throw error;
    }
  }

  async analyzeLessonContent(lessonContent, onChunk = null) {
...
Trả lời bằng tiếng Việt, chi tiết và có cấu trúc rõ ràng.`;
    ...
    try {
      ...
      const analysis = this.cleanupAIResponse(fullText);
