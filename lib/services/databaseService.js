  ...
  async getLessonDetailedStatistics(lessonId) {
    try {
      ...
      if (lessonQuestions && Array.isArray(lessonQuestions) && lessonQuestions.length > 0) {
        questionStats = lessonQuestions.map((lessonQ, index) => {
          let correct = 0;
          let incorrect = 0;
          ...
          results?.forEach(result => {
            if (result.questions && Array.isArray(result.questions)) {
              // Find the matching question in this specific student result by its text content
              // This handles cases where questions are shuffled or selected from a pool
              ...
              if (studentQ) {
                completedCount++;
                // Check both property names (isCorrect is the modern name, correct is fallback)
