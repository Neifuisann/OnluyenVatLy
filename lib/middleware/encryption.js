...
const { isEncryptionEnabled } = require('./encryptionToggle');
...
const encryptResponse = (options = {}) => {
  ...
  return async (req, res, next) => {
    ...
    // Skip encryption if explicitly disabled or globally disabled
    if (skipEncryption || !globalEncryptionEnabled) {
      return next();
    }
...
const decryptRequest = (options = {}) => {
  ...
  return async (req, res, next) => {
    ...
    // Skip decryption if explicitly disabled or globally disabled
    if (skipDecryption || !globalEncryptionEnabled) {
      return next();
    }
