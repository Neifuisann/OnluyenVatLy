...
const { getStockImage } = require('../utils/stockImages');
...
class LessonController {
  ...
  getAllLessons = asyncHandler(async (req, res) => {
    ...
    // Apply stock image if lesson has no image
    if (result.lessons && Array.isArray(result.lessons)) {
      result.lessons = result.lessons.map(lesson => {
        if (!lesson.lessonImage && !lesson.image_url) {
          lesson.lessonImage = getStockImage(lesson.id);
        }
        return lesson;
      });
    }
  ...
  getLessonById = asyncHandler(async (req, res) => {
    ...
    // Check if the user is an admin to bypass question pool filtering
    const isAdmin = sessionService.isAdminAuthenticated(req);
    const lesson = await databaseService.getLessonById(id, isAdmin);
    ...
    // Apply stock image if lesson has no image
    if (!lessonWithProgress.lessonImage && !lessonWithProgress.image_url) {
      lessonWithProgress.lessonImage = getStockImage(lessonWithProgress.id || id);
    }
  ...
  updateLesson = asyncHandler(async (req, res) => {
    ...
    if (updateData.regenerateAiSummary || 
      ...
      try {
        // Get existing lesson data if needed (admin is updating, so bypass question pool)
        const existingLesson = await databaseService.getLessonById(id, true);
  ...
  // Get students who got a specific question wrong (admin only)
  getStudentsWhoGotQuestionWrong = asyncHandler(async (req, res) => {
    ...
    const result = await databaseService.getStudentsWhoGotQuestionWrong(
      lessonId,
  ...
  getStudentRankings = asyncHandler(async (req, res) => {
    ...
    const transcripts = results.map(result => {
      // Handle both snake_case (DB) and camelCase field names
      const totalPts = result.total_points !== undefined ? result.total_points
                     : result.totalPoints !== undefined ? result.totalPoints
                     : null;
      const scoreVal = result.score;
      const scoreStr = (scoreVal !== undefined && scoreVal !== null && totalPts)
        ? `${Math.round((scoreVal / totalPts) * 100)}%`
        : '0%';
      return {
