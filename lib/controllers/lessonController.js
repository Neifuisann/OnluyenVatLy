...
class LessonController {
  ...
  getStudentsWhoGotQuestionWrong = asyncHandler(async (req, res) => {
    ...
    const { questionIndex, optionIndex } = req.query;
  ...
  // Get attempts for a student in a lesson (admin only)
  getLessonStudentAttempts = asyncHandler(async (req, res) => {
    const { id: lessonId } = req.params;
    ...
    const result = await databaseService.getLessonStudentAttempts(lessonId, studentId);
    res.json(result);
  });
