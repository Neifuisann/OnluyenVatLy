...
class RatingController {
  ...
  // Search ratings by student name
  searchRatings = asyncHandler(async (req, res) => {
    const { q = '', filter = 'all', limit = 100 } = req.query;
    ...
    const results = await databaseService.searchRatings(q.trim(), filter, parseInt(limit));

    // Add tier information to each result
    const resultsWithTiers = results.map(rating => ({
      ...rating,
      tier: ratingService.getRatingTier(rating.rating)
    }));
    ...
    res.json({
      success: true,
      ratings: resultsWithTiers,
      count: resultsWithTiers.length,
      query: q.trim()
    });
  });
}
